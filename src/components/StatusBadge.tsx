...
export const StatusBadge = ({ status, className }: { status: CaseStatus; className?: string }) => (
  <span className={cn('inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-xs font-medium', statusStyles[status], className)}>
    {status === 'emergency' && <span className="h-1.5 w-1.5 rounded-full bg-red-500 animate-pulse" />}
