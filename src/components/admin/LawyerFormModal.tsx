...
import { LEGAL_CATEGORIES, type LegalCategory, type Lawyer } from '@/types';
...
const allCategories = Object.entries(LEGAL_CATEGORIES) as [LegalCategory, string][];
...
export const LawyerFormModal = ({ open, onClose, lawyer, onSave }: LawyerFormModalProps) => {
  ...
  useEffect(() => {
    if (lawyer) {
      ...
      setName(''); setEmail(''); setPhone(''); setBarCouncilId('');
      setDegree(''); setExperience(0); setBio('');
      setSpecializations([]); setIsVerified(false); setIsAvailable(true);
    }
  }, [lawyer, open]);
  ...
  const handleSave = () => {
    ...
    onSave({ name, email, phone, barCouncilId, degree, experience, bio, specializations, isVerified, isAvailable });
    onClose();
  };
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Input value={name} onChange={e => setName(e.target.value)} placeholder="Adv. Name" />
            </div>
            <div className="space-y-1.5">
              <Label>Email</Label>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={barCouncilId} onChange={e => setBarCouncilId(e.target.value)} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave}>{lawyer ? 'Save Changes' : 'Add Lawyer'}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
